...
import {first, isEmpty, isEqual, toArray, isUndefined} from "lodash-es";
import {ROCrate} from 'ro-crate';
import {recordResolve} from '../controllers/recordResolve.js';
import {getLogger} from "../services/logger.js";
import {getRootConformsTos} from "../controllers/rootConformsTo.js";
import {getFile, getCrate} from "../controllers/record.js";
    ...
    try {
      ...
      if (rootConformsTos.length > 0) {
        for (let rootConformsTo of rootConformsTos) {
          ...
          //TODO: add version
          const rawCrate = await getCrate({repository: this.repository, crateId: col['crateId'], raw: true});
  ...
  async indexObjects({parent, crateId, _memberOf}) {
    ...
    try {
      ...
      for (let member of members) {
        ...
        //TODO: add version
        const rawCrate = await getCrate({repository: this.repository, crateId: member['crateId'], raw: true});
        this.crate = new ROCrate(rawCrate, {alwaysAsArray: true, resolveLinks: true});
