...
export async function filterResults({userId, results, configuration}) {
  ...
  for (let index = 0; index < results?.hits?.hits.length; index++) {
    ...
    const pass = await checkIfAuthorized({userId, license: license['@id'], configuration});
    if (!pass) {
      ...
      results.hits.hits[index]._source = {
        ...
        //name: results.hits.hits[index]?._source?.name, // Do not include the name of the object, because it may contain sensitive information
